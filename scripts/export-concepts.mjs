...
try {
  ...
  for (const option of options) {
    ...
    if (option.key === 'a' && state.verticalStoryHeadings.join('|') !== 'How we started|Where we are|Where we’re going') throw new Error(`${option.name}: story sections are not in the requested order`)
